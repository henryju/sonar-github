...
public class PullRequestIssuePostJob implements PostJob {
  ...
  private Map<InputFile, Map<Integer, StringBuilder>> processIssues(GlobalReport report, Iterable<PostJobIssue> issues) {
    ...
    StreamSupport.stream(issues.spliterator(), false)
      ...
      .forEach(i -> {
        processIssue(report, commentToBeAddedByFileAndByLine, i);
      });
  ...
  private void processIssue(GlobalReport report, Map<InputFile, Map<Integer, StringBuilder>> commentToBeAddedByFileAndByLine, PostJobIssue issue) {
    ...
    if (gitHubPluginConfiguration.tryReportIssuesInline() && issue.inputComponent() != null && issue.inputComponent().isFile()) {
      reportedInline = tryReportInline(commentToBeAddedByFileAndByLine, issue, (InputFile) issue.inputComponent());
    }
    report.process(issue, pullRequestFacade.getGithubUrl(issue.inputComponent(), issue.line()), reportedInline);
  }
